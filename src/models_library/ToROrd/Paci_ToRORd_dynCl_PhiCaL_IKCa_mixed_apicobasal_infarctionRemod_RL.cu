...
//Adapted from https://github.com/rsachetto/MonoAlg3D_C/tree/master/src/models_library/ToROrd by Leto Riebel
//Uses Rush Larsen by default, uncomment the 2 "Uncomment if using euler" lines if using Euler

extern "C" SET_ODE_INITIAL_CONDITIONS_GPU(set_model_initial_conditions_gpu) {
    size_t pitch_h;
    uint8_t use_adpt_dt = (uint8_t)solver->adaptive;

    log_info("Using Paci_ToRORd_dynCl_mixed_endo_mid_epi GPU model. If you want to use the stem cell model, please edit the .common file\n");
...
inline __device__ void RHS_gpu(real *a_, real *b_, real *sv, real *rDY, real stim_current, int thread_id, real dt, size_t pitch, bool use_adpt_dt, int is_paci, int layer, int infarct_zone, int infarct_stage, real apicobasal, real *current_scaling) {
    ...
    //Applied globally, i.e. because of drug application
    ...
    //Applied to the remote, i.e. healhty zone  (i.e. infarct_zone = 0)
    ...
    //Applied to the infarct center (i.e. infarct_zone = 1)
    ...
    //Applied to the border zone (i.e. infarct_zone = 2)
    ...
    //Applied to the atrial-like iPSC-CMs (i.e. is_paci = 2)
    ...
    if(is_paci > 0) {
        ...
        cai = *((real *)((char *)sv + pitch * 2) + thread_id);     
        d = *((real *)((char *)sv + pitch * 3) + thread_id);       
        f1 = *((real *)((char *)sv + pitch * 4) + thread_id);      
        f2 = *((real *)((char *)sv + pitch * 5) + thread_id);     
        fCa = *((real *)((char *)sv + pitch * 6) + thread_id);      
        Xr1 = *((real *)((char *)sv + pitch * 7) + thread_id);      
        Xr2 = *((real *)((char *)sv + pitch * 8) + thread_id);     
        Xs = *((real *)((char *)sv + pitch * 9) + thread_id);       
        h = *((real *)((char *)sv + pitch * 10) + thread_id);      
        j = *((real *)((char *)sv + pitch * 11) + thread_id);       
        m = *((real *)((char *)sv + pitch * 12) + thread_id);      
        Xf = *((real *)((char *)sv + pitch * 13) + thread_id);
        q = *((real *)((char *)sv + pitch * 14) + thread_id);
        r = *((real *)((char *)sv + pitch * 15) + thread_id);       
        nai = *((real *)((char *)sv + pitch * 16) + thread_id);     
